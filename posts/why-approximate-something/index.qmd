...
The fascination with AI is understandable, but it must not lead us to abandon sound engineering principles. When we design a system, we must consider all possible outcomes. Too often, we fall into the trap of "solutionising"—jumping to a solution for a problem that hasn't been well defined. This is precisely what happens when we default to "let's use AI" without first asking if a deterministic approach is better.
