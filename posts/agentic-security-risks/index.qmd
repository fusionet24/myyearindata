...
Clawdbot (now rebranded to Moltbot after Anthropic's cease and desist) is an open-source macOS application created by Peter Steinberger that gives Claude full control of your computer. It connects to WhatsApp, Telegram, Signal, iMessage, and email through MCP servers, can read and write files, execute terminal commands, browse the web, and interact with your desktop. Think of it as giving an AI agent the keys to everything on your machine.

The hype was extraordinary. In days it accumulated over 60,000 GitHub stars, making it one of the fastest-growing repos ever. Developers were drawn to the promise of a truly capable AI assistant that could actually *do* things rather than just chat. The problem? Every one of those capabilities is also an attack surface. And Peter, as a solo maintainer of a hobby project, suddenly found himself fielding bug bounty demands and security vulnerability reports.
:::
